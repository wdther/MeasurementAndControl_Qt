...
{    resize(400,400);
     ...
     m_endAngle = 30; //从x轴正方向为起点

     m_refSize = 200;
     m_radius = m_refSize /2;
    SpeedPanelInit(0,220);
...
void CstmSpeedPanel::SpeedPanelInit(int minSpeed,int maxSpeed)
{
    m_minSpeed = minSpeed;   //由于绘制刻度，需要为5倍数
    m_maxSpeed = maxSpeed; //由于绘制刻度，需要为5倍数
    m_curSpeed = m_minSpeed;
    //速度对应的角度
    m_anglePerVel = (360.0 - (m_startAngle - m_endAngle)) / (m_maxSpeed - m_minSpeed);
    space=(m_maxSpeed-m_minSpeed)/10;
...
void CstmSpeedPanel::paintEvent(QPaintEvent */*event*/)
{
    ...
    drawNumberIndicator(painter);//绘制指示数字，即0 20 40 60 那些
...
void CstmSpeedPanel::drawNumberIndicator(QPainter &painter)
{
    ...
    for (int i = m_minSpeed; i <= m_maxSpeed; i+= space)//每隔20Km设置一个数字
    {
...
void CstmSpeedPanel::drawDividing(QPainter &painter)
{
    ...
    for (int i = m_minSpeed; i <= m_maxSpeed; i +=5) //原来的刻度距离是5，我修改了
    {
        if (i >= 200){ //绘制红色
            pen.setColor(Qt::red);
            painter.setPen(pen);
...
void CstmSpeedPanel::drawNumberSpeed(QPainter &painter)
{
    painter.save();
    painter.setPen(Qt::white);
    QString speed = QString("%1 r/s").arg(m_curSpeed);
    QFontMetricsF fm(this->font());
